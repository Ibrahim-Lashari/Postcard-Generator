...
states.onchange = function() {
    ...
    //select the innerHTML of element with the ID of 'postcard' and changes the background image to the state chosen
    document.getElementById('postcard').style.backgroundImage = 'url(img/' + states.value + '@2x.jpg)'

    //select the innerHTML of element with the ID of 'state-heading' and changes the text to the name of the selected index (state name)
    document.getElementById('state-heading').innerHTML = states.options[states.selectedIndex].text;

    //select the innerHTML of element with the ID of 'greeting' and changes the text to 'Greetings from:'
